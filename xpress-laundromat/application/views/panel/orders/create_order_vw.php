                        ...
                        <div class="panel-body">
                            ...
                            <div class="col-sm-12" id="orderform" style="display: none;">
                                ...
                                <div class="form-group row">
									...
									<div class="col-sm-12 no-padding margin-top-5 m-b-40"  id="couponCntnr">
										 <div class="col-sm-8">
										...
										<div class="err" id="coupon_code_err"></div>
                                    </div>
...
<script>
    $(document).ready(function(){
		...
		$('#couponBtn').click(function(){
			...
			var total_a= $('#total_price').val();
			var data = $('#orderFrm').serialize();
			var url = '<?=BASE?>panel/orders/checkCouponCode';
			$.post(url,data,function(success){
				var res = JSON.parse(success);
                if(res.status)
                {   //console.log(parseInt(total_a));
                    var discount = ($('#total_price').val()*res.discount_percent)/100;
                    var final_amount = 0;
                    if(parseInt(total_a) > discount)
                    {
                        final_amount = $('#total_price').val()-discount;
                    ...
                    $('#couponCntnr').hide();
					$("#coupon_text").show();
                    $('#coupon_text').html('Coupon code <b>'+$('#coupon_code').val()+'</b> applied. <a id="remove_coupon" href="javascript:void(0)">Remove Coupon</a>').show();
                    $('#total_price').html(final_amount);
                    $('#discount_amount').html('-'+discount);
					$("#coupon_code_err").hide();
                }else{
					$("#coupon_code_err").show();
                    $('#coupon_code_err').html(res.error);
                }
            })
