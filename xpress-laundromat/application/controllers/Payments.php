...
//                print_r($pagedata);die;
	...
	function response()
	{
		...
		if($res['res'])
		{
			...
			if (isset($responseParamList) && count($responseParamList)>0)
			{
				...
				$val = ['mihpayid' => 0,'transcation_id'=> $responseParamList['TXNID'],'amount'=> intval($responseParamList['TXNAMOUNT']),'user_id'=> $this->session->user_id,'payment_method'=> 'Paytm','recharge_by'=> 0,'payment_status'=> $payment_status,'created'=> strtotime($responseParamList['TXNDATE']),'fail_reason'=> $fail_reason,'description'=> $description,'order_id'=>$this->session->paytm_order_id];
				$data = ['val'=>$val,'table'=>'tbl_transcations'];
                $this->common->add_data($data);
	...
	{
		$order_id=$this->session->paytm_order_id;
		$this->db->select('*');
        $res = $this->db->from('tbl_transcations')->where(['order_id'=>$order_id])->get()->result();
		if(empty($res))
		{
			$responseParamList=file_get_contents(BASE."payments/paytmResponse");
			if(isset($responseParamList) && count($responseParamList)>0)
			{
				if($responseParamList['STATUS']=='TXN_SUCCESS' && $responseParamList['RESPCODE']=='01')
				{
				...
				$val = ['mihpayid' => 0,'transcation_id'=> $responseParamList['TXNID'],'amount'=> intval($responseParamList['TXNAMOUNT']),'user_id'=> $this->session->user_id,'payment_method'=> 'Paytm','recharge_by'=> 0,'payment_status'=> $payment_status,'created'=> strtotime($responseParamList['TXNDATE']),'fail_reason'=> $fail_reason,'description'=> $description,'order_id'=>$order_id];
				$data = ['val'=>$val,'table'=>'tbl_transcations'];
				$this->common->add_data($data);
				$msg=$responseParamList['RESPMSG'];
			}
