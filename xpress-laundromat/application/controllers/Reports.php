...
<!--                    <td><?php if($trans->payment_status==2){echo $trans->fail_reason;}elseif($trans->payment_status==3){echo 'Incompelete Payment';}?></td>-->
    ...
    function export_sales()
    {
		...
		$hostel_name = $product->hostel_name."\n".$product->room_no;
                ...
                $this->excel->getActiveSheet()->setCellValue('I'.$j, date('d-m-Y H:i a',$product->created));
    ...
    function download_report()
    {
        ...
        foreach($colleges as $college)
        {
            ...
            if(!empty($res))
            {
                ...
                foreach($res as $product)
                {
					$online_data=0;$offline_data=0;
					if($product->online_data!="" && $product->online_data!="0")
					{ 
						$online_data=$product->online_data;
					}
					if($product->online_data!="" && $product->online_data!="0")
					{ 
						$offline_data=$product->offline_data;
					}
                    $this->excel->getActiveSheet()->setCellValue('A'.$j, $i);
                    ...
                    $this->excel->getActiveSheet()->setCellValue('F'.$j, ($online_data + $product->extra_amount));
                    $this->excel->getActiveSheet()->setCellValue('G'.$j, $offline_data);
                    $this->excel->getActiveSheet()->setCellValue('H'.$j, $product->extra_amount);
                    $this->excel->getActiveSheet()->setCellValue('I'.$j, ($product->total_amount + $product->extra_amount));
                    $i++;$j++;
                }
            }
            $m++;
