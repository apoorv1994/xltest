...
<!--                <td> <?=$users->last_order_day?></td>-->
    ...
    function ajax_transcation($id=0)
    {
        ...
        $data=['val'=>'a.*,b.id as user_id,b.firstname as name','table'=>'tbl_transcations a','where'=>$where,'where2'=>$where2,'orderby'=>'id','orderas'=>'DESC','start'=>$id,'limit'=>$perpage];
        $join=['table'=>'tbl_users b','on'=>'a.user_id=b.id','join_type'=>''];
        $res = $this->common->get_join($data,$join);//echo $this->db->last_query();die;
        if($res['res'])
        {
            $i=1+$id;
            ...
            <tr class="odd gradeX">
                    ...
                    <td><?=$trans->payment_method=='Payu'?'Online':'Offline'?></td>
...
<!--                    <td><?php if($trans->payment_status==2){echo $trans->fail_reason;}elseif($trans->payment_status==3){echo 'Incompelete Payment';}?></td>-->
    ...
    function export_wallet()
    {
        ...
        if($res['res'])
        {
            ...
            foreach($res['rows'] as $product)
            {
                ...
                $this->excel->getActiveSheet()->setCellValue('G'.$j, $payment->payment_method=='Payu'?'Online':'Offline' );
                $i++;$j++;
            }
            $this->excel->stream('report.xls');
        }
    ...
    function export_sales()
    {
        ...
        if($from != $to)
        {
            $where2 .= "(from_unixtime(book_date,'%m-%d-%Y') > '". $from."' and from_unixtime(book_date,'%m-%d-%Y') <= '". $to."' ) ";
        }else{
            $where2 .= "(from_unixtime(book_date,'%m-%d-%Y') = '". $from."' ) ";
        }
...
//        print_r($res);die;
        if($res)
        {
            ...
            $this->excel->getActiveSheet()->setCellValue('Q1', 'Total Amount');
            ...
            foreach($res as $product)
            {
                ...
                $this->excel->getActiveSheet()->setCellValue('I'.$j, date('d-m-Y',$product->created));
                ...
                $this->excel->getActiveSheet()->setCellValue('Q'.$j, $product->final_amount?$product->final_amount:$product->total_amount );
